...
p.CemitterCollector = 2; % f farad
p.Cparasitic = 0.05; % 0.1 f farad
% p.Radius = 1; % 1 nm?
p.Radius = 10; % 1 nm
...
X = ForwardEulerNew(@eval_f2, x0,p,u,t_start,t_stop,timestep);
...
plot(t, X(5+4*p.col,:))
plot(t, X(6+4*p.col,:))
plot(t, X(5+2*(p.row-1)*p.col, :))
plot(t, X(6+2*(p.row-1)*p.col, :))
legend('vodd1','veven1','vodd2','veven2','vodd3','veven3')
title('Internal node voltages over time','FontSize', 8)
xlabel('Time (fs)')
ylabel('Voltage (V)')
hold off;
